...
from .schema import (  # noqa: F401
    ...
    TYPE_NONE, TYPE_MAX,
    STATUS_NONE, STATUS_FAINT, STATUS_UNKNOWN, STATUS_MAX,
    WEATHER_NONE, WEATHER_MAX,
    FIELD_NONE, FIELD_MAX,
    CATEGORY_NONE, CATEGORY_PHYSICAL, CATEGORY_SPECIAL, CATEGORY_STATUS, CATEGORY_UNKNOWN,
    EFFECT_NONE, EFFECT_UNKNOWN,
    NUM_ACTIONS, NUM_POKEMON_SLOTS, NUM_MOVES_PER_POKEMON,
    POKEMON_NUM_FEATURES, POKEMON_NUM_LEN,
    ...
    SLOT_REVEALED_OPP_3, SLOT_REVEALED_OPP_4, SLOT_REVEALED_OPP_5,
)
...
POKEMON_CAT_FEATURES = [
    "species", "type_1", "type_2", "status", "effect",
...
POKEMON_MASK_FEATURES = [
    "valid", "fainted", "moves_revealed", "hp_known",
...
class Gen3RomBattleState:
    ...
    pokemon: List[Gen3PokemonFeatures] = field(
        default_factory=lambda: [Gen3PokemonFeatures() for _ in range(NUM_POKEMON_SLOTS)]
    )
    ...
    def to_tensors(self) -> dict:
        ...
        for i, p in enumerate(self.pokemon):
            ...
            pokemon_mask[i] = [
                ...
                int(p.item_revealed), int(p.ability_revealed),
            ]
