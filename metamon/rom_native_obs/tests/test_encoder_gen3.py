...
from metamon.rom_native_obs.schema_gen3 import (
    Gen3RomBattleState, NUM_POKEMON_SLOTS, NUM_ACTIONS,
    POKEMON_CAT_LEN, POKEMON_MASK_LEN, POKEMON_NUM_LEN,
    SLOT_PLAYER_ACTIVE, SLOT_OPPONENT_ACTIVE, SLOT_SWITCH_0,
    SPECIES_MAX_GEN3, MOVE_MAX_GEN3, ABILITY_MAX_GEN3, ITEM_MAX_GEN3,
    SIDE_COND_SPIKES,
)
...
def _mv(name="thunderbolt", t="electric", cat="special", bp=95):
    return UniversalMove(name=name, move_type=t, category=cat, base_power=bp,
                         accuracy=1.0, priority=0, current_pp=24, max_pp=24)


def _mon(name="salamence", types="dragon flying", item="leftovers", ability="intimidate",
         hp=1.0, status="nostatus", moves=None, lvl=100):
    if moves is None:
        moves = [_mv()]
    return UniversalPokemon(
        name=name, hp_pct=hp, types=types, item=item, ability=ability, lvl=lvl,
        status=status, effect="noeffect", moves=moves,
        atk_boost=0, spa_boost=0, def_boost=0, spd_boost=0, spe_boost=0,
        accuracy_boost=0, evasion_boost=0,
        base_atk=135, base_spa=110, base_def=80, base_spd=80, base_spe=100, base_hp=95,
        hp_stat=-1, atk_stat=-1, def_stat=-1, spa_stat=-1, spd_stat=-1, spe_stat=-1,
        tera_type="notype", base_species=name,
    )


def _state(opp_item="leftovers", opp_ability="intimidate", opp_moves=None,
           player_conditions="noconditions", forced_switch=False, nswitch=2):
    player = _mon("metagross", types="steel psychic", item="choiceband", ability="clearbody")
    ...
    switches = [_mon("skarmory", types="steel flying"), _mon("blissey", types="normal")][:nswitch]
    return UniversalState(
        ...
        weather="noweather", battle_field="nofield",
        forced_switch=forced_switch, battle_won=False, battle_lost=False,
        can_tera=False, opponent_teampreview=[],
    )
...
class TestGen3Mappings(unittest.TestCase):
    def test_species_bounds(self):
        ...
        self.assertTrue(all(1 <= v <= SPECIES_MAX_GEN3 for v in mg.SPECIES_NAME_TO_ID.values()))
    ...
    def test_move_bounds(self):
        ...
        self.assertTrue(all(1 <= v <= MOVE_MAX_GEN3 for v in mg.MOVE_NAME_TO_ID.values()))
    ...
    def test_ability_rom_canonical(self):
        ...
        self.assertTrue(all(1 <= v <= ABILITY_MAX_GEN3 for v in mg.ABILITY_NAME_TO_ID.values()))
    ...
    def test_item_expansion_enum(self):
        ...
        self.assertTrue(all(1 <= v <= ITEM_MAX_GEN3 for v in mg.ITEM_NAME_TO_ID.values()))
...
class TestGen3Encoder(unittest.TestCase):
    ...
    def test_opp_item_ability_revealed_in_full_obs_replay(self):
        ...
        enc = Gen3RomObservationEncoder()
        t = enc.encode(_state(opp_item="leftovers", opp_ability="intimidate")).to_tensors()
        ...
        self.assertEqual(t["pokemon_mask"][SLOT_OPPONENT_ACTIVE][5], 1)  # ability_revealed

    def test_opp_hidden_then_revealed_memory(self):
        enc = Gen3RomObservationEncoder()
        # first: opponent item/ability unknown
        t1 = enc.encode(_state(opp_item="unknownitem", opp_ability="unknownability")).to_tensors()
        ...
        t2 = enc.encode(_state(opp_item="leftovers", opp_ability="unknownability")).to_tensors()
        self.assertEqual(t2["pokemon_cat"][SLOT_OPPONENT_ACTIVE][9], 472)
        # back to unknown display, but memory retains the reveal
        t3 = enc.encode(_state(opp_item="unknownitem", opp_ability="unknownability")).to_tensors()
    ...
    def test_opp_moves_hidden_until_revealed(self):
        ...
        self.assertEqual(t1["pokemon_mask"][SLOT_OPPONENT_ACTIVE][2], 0)  # moves_revealed=0
        t2 = enc.encode(_state(opp_moves=[_mv("crunch", "dark", "physical", 80)])).to_tensors()
        self.assertEqual(t2["pokemon_mask"][SLOT_OPPONENT_ACTIVE][2], 1)
        self.assertGreater(t2["pokemon_cat"][SLOT_OPPONENT_ACTIVE][5], 0)  # move id present
